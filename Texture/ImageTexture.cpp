...
ImageTexture::ImageTexture(Image* _image_ptr)
	:	Texture(),
		hres(_image_ptr->getHres()),
		vres(_image_ptr->getVres()),
...
RGBColor														
ImageTexture::getColor(const ShadeRec& sr) const {	
	int row;
	int column;
		
	if (mapping_ptr)
		mapping_ptr->getTexelCoordinates(sr.local_hit_point, hres, vres, row, column);
	else {
		row 	= (int)(sr.v * (vres - 1));  	
