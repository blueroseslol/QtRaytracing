...
#include "Grid.h"
#include <QDebug>
//#include "Geometry/Triangles/MeshTriangle.h"
//#include "Geometry/Triangles/FlatMeshTriangle.h"
...
Grid::Grid(Mesh* _mesh_ptr):Compound(),
		nx(0),
		ny(0),
		nz(0),
		mesh_ptr(_mesh_ptr),
...
// comment this out if you don't want to use it
    ...
    qDebug() << "num_cells =" << num_cells;
    qDebug() << "numZeroes = " << num_zeroes << "  numOnes = " << num_ones << "  numTwos = " << num_twos;
    qDebug()<< "numThrees = " << num_threes << "  numGreater = " << num_greater << endl;
...
//void Grid::readPlyFile(char* file_name, const int triangle_type) {
//    // Vertex definition
...
//        qDebug() << "element name  " << elem_name << "  num elements = " << num_elems << "  num properties =  " << nprops;
...
//                    triangle_ptr->computeNormal(reverseNormal);
...
//                    triangle_ptr->computeNormal(reverseNormal); 	// the "flat triangle" normal is used to compute the average normal at each mesh vertex
...
//            normal += objects[mesh_ptr->vertex_faces[index][j]]->getNormal(Point3D());
...
//    qDebug() << "finished constructing normals";
//}
